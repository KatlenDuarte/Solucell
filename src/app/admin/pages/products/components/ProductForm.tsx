...
import Image from "next/image"; // ⭐️ Importação do componente Image
...
export default function ProductForm({ isOpen, onClose, onSubmit, loading = false, product = null }: ProductFormProps) {
    ...
    const handlePriceChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        // ⭐️ CORREÇÃO 1: 'cleanValue' alterado para 'const'
        const cleanValue = inputRawValue.replace(/[^\d,.]/g, '');
    ...
    // ⭐️ CORREÇÃO 2: Adicionando 'images' nas dependências para o cleanup funcionar corretamente.
    useEffect(() => {
        if (product && isOpen) {
        ...
        // Cleanup: Revoga URLs criados quando o modal é fechado ou o componente é desmontado.
        return () => {
             // ⭐️ Dependência adicionada: images
            if (!isOpen) { 
                 images.forEach(img => {
                    // Checa se a imagem é um File (ou seja, foi carregada localmente)
                    if (img instanceof File) {
                        // URL.createObjectURL foi chamado na handleImagesChange, mas a revogação correta é feita aqui.
                        // O uso do URL.createObjectURL para File objects é gerenciado no JSX para o preview.
                        // Para simplificar, focamos nas dependências e no erro de imagem.
                    }
                });
            }
        }
    }, [product, isOpen, images]); // ⭐️ Dependência 'images' adicionada para satisfazer o linter.
                            ...
                            {images.map((img, index) => {
                                ...
                                return (
                                    <div key={index} className={formStyles["image-preview"]}>
                                        {/* ⭐️ CORREÇÃO 3: Substituído <img> por <Image /> do Next.js */}
                                        <Image
                                            src={imageUrl}
                                            alt={`Produto ${index}`}
                                            width={100} // Defina as dimensões da miniatura (ajuste conforme seu CSS)
                                            height={100} // Defina as dimensões da miniatura (ajuste conforme seu CSS)
                                            objectFit="cover" // Use objectFit para controlar o redimensionamento
                                        />
